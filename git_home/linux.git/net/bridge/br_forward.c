...
modifyMcast2Ucast(struct sk_buff *skb,unsigned char *mac)
{
	  ...
	  if ( skb == NULL || mac == NULL)
		  return NULL;
      mSkb=skb_copy(skb,GFP_ATOMIC);
	  if ( mSkb == NULL)
		  return NULL;
      struct ethhdr *ethernet = eth_hdr(mSkb);
	  if ( ethernet == NULL)
		  return NULL;
      //mSkb=skb_copy(skb,GFP_ATOMIC);
	  memset(ethernet->h_dest,0,6);
...
static void custom_bootp_lease_time(struct sk_buff *skb)
{
 ...
 // udph = (struct udphdr *)(((unsigned char *)iph) + (iph->ihl<<2));
    ...
    unsigned char *type;
    u_int32_t lease;
    if( !strncmp(skb->dev->name, "ath01", 5) || !strncmp(skb->dev->name, "ath11", 5) )
        lease = htonl(1800); // 30 minutes
    else
        lease = htonl(86400); // 1 day
...
static void __br_deliver(const struct net_bridge_port *to, struct sk_buff *skb)
{
	...
	list_for_each_entry_rcu(p, &to->br->port_list, list){
		if(!strncmp(p->dev->name, "ath01", 5) || !strncmp(p->dev->name, "ath11", 5))
			guest_network = 1;
	}
	/* solve the dhcp packet which out from wireless guest network port */
	if (guest_network && !strncmp(to->br->dev->name, "br0", 3)) {
		struct ethhdr *ethernet = eth_hdr(skb);
