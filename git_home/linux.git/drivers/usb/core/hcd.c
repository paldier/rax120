...
extern void detect_usb0_data(void);
...
int usb_hcd_submit_urb (struct urb *urb, gfp_t mem_flags)
{
	...
	if (is_root_hub(urb->dev)) {
		...
		if (likely(status == 0)) {
#ifdef CONFIG_DNI_TRAFFIC_BLINK
			if(dev_name(&urb->dev->dev)){
				if((strncasecmp(dev_name(&urb->dev->dev), "1-1", 3) == 0) || (strncasecmp(dev_name(&urb->dev->dev), "2-1", 3) == 0))
					detect_usb0_data();
				if((strncasecmp(dev_name(&urb->dev->dev), "3-1", 3) == 0) || (strncasecmp(dev_name(&urb->dev->dev), "4-1", 3) == 0))
					detect_usb1_data();
			}
#endif
