...
extern char console_buf[4096];
extern unsigned int console_index;
...
#ifdef CONFIG_DNI_CONSOLE_REDIRECT_DEBUG
	if (console_enable == 1) {
		if (console_index == 0 || (count + console_index) >= (MAX_BUFF_COUNT - 3)) {
		...
		for (i = 0; i < count; i++) {
			...
			if (!(buf[i] == 0x5 || buf[i] == 0x7 || buf[i] == 0x1B)) {
				console_buf[console_index] = (char)buf[i];
				console_index++;
			}
		}
	}
