...
static void select_ntp_servers(char **primary, char **secondary)
{
	 ...
	 * Fix bug: 100377
	 * */
	if (strstr(config_get("time_zone"), "GMT+3:30") != NULL)
...
int main(int argc, char *argv[]) {
	...
	char manual_ntp_server_primary[128]={0};  // The length of manual server have checked with GUI,it's less than 128 bytes.
	char manual_ntp_server_secondary[128]={0};
	...
	if (argc == 1) {
		...
		if(strcmp(manual_ntp,"1") != 0){
			...
			strcpy(manual_ntp_server_primary, config_get("manual_ntp_server"));
			strcpy(manual_ntp_server_secondary, hostname);
			hostname=manual_ntp_server_primary;
			sec_host=manual_ntp_server_secondary;
	...
	/* Record ntpclient local port for RA Object - "ntpPortNumber" */
