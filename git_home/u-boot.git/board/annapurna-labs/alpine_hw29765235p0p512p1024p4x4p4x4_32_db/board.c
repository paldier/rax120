...
int checkboard(void)
{
	printf("U-boot dni1 V2.0 for DNI HW ID: 29765235 NOR flash 0MB; NAND flash 512MB; RAM 1024MB; 1st Radio 4x4; 2nd Radio 4x4\n");
	return 0;
}
#endif
...
int board_match_image_hw_id (ulong fw_image_addr)
{
	...
	/* Only check first 27 chars 29765235+0+512+1024+4x4+4x4 */
...
int do_nandtest (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	size_t start ;
	nand_info_t *nand;
	unsigned int  current ,blocksize, end ,erasesize ,badblockcnt ,chkaddr;
	char runcmd[256];
	int loops , rev;
	
	start = 0x580000 , end = 0x20000000 ;
	erasesize = end - start;
	chkaddr = 0x7f00000;
	blocksize = CONFIG_SYS_FLASH_SECTOR_SIZE ;
	nand = &nand_info[0];
	printf("nandsize: 0x%" PRIx64 "\n", nand->size);

	if (argc < 2) {
		loops = 1;
	} else {
		loops = simple_strtol(argv[1], NULL, 16);
		if (loops <= 0){
			printf("Usage:\n%s\n", cmdtp->usage);
			return 0;
		}
	...
	while(loops){
		//erase to 0xff
			badblockcnt = 0;
			snprintf(runcmd, sizeof(runcmd), "nand erase 0x%lx 0x%lx", start, erasesize);
			run_command(runcmd,0);

		for (current = start ; current < end ;current += 0x20000){

			while (current < nand->size &&
			       nand_block_isbad(nand, current)) {
				printf("Skipping bad block 0x%08llx\n", current);
				badblockcnt ++;
				current += CONFIG_SYS_FLASH_SECTOR_SIZE;
			}

			if (current >= nand->size - badblockcnt * CONFIG_SYS_FLASH_SECTOR_SIZE) {
				printf("current address arrive flash end address: %" PRIx64 "\n", nand->size - badblockcnt * CONFIG_SYS_FLASH_SECTOR_SIZE);
				break;

			}
			snprintf(runcmd, sizeof(runcmd), "mem_pattern_set 0x8000000 0x4000 0x0 0x0");
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "nand read 0x8000000 0x%lx 0x%lx",current,blocksize);
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "crc32 0x8000000 0x%lx 0x%lx",  blocksize ,chkaddr);
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "if itest *0x%lx != 154803cc ; then setenv testfail 1 ; echo crc32 checksum fail;fi",chkaddr);
			run_command(runcmd,0);
			if(getenv("testfail"))
				goto testfail;
		}
		...
		for (current = start ; current < end ;current += 0x20000){
			...
			snprintf(runcmd, sizeof(runcmd), "nand write 0x8000000 0x%lx 0x%lx", current, blocksize);
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "nand read 0x8000000 0x%lx 0x%lx", current, blocksize);
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "crc32 0x8000000 0x%lx 0x%lx",  blocksize, chkaddr);
			run_command(runcmd,0);
			snprintf(runcmd, sizeof(runcmd), "if itest *0x%lx != 7ee8cdcd ; then setenv testfail 1 ; echo crc32 checksum fail ;fi", chkaddr);
			run_command(runcmd,0);
			if(getenv("testfail"))
				goto testfail;
		}
	...
	printf("NAND flash test pass! \n");
	return 0;

testfail : 
	printf("NAND flash test fail!\n");
	snprintf(runcmd, sizeof(runcmd), "setenv testfail");
	run_command(runcmd,0);
	return 0;
}

U_BOOT_CMD(
        nandtest,2, 0, do_nandtest,
        "nandtest - nand flash stress test\n",
	"<loops> - nand flash stress test for given loops"
);
