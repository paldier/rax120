...
version_string="$0 v0.1.2-ipq807x.ilq.7.0-spf.7.0"
...
if [ "$1" ]; then
    ...
    if [ "$bits" = "64" ]; then
        cp -t "$work_dir"/IPQ8074.ILQ.7.0/common/build/ipq_x64/ $@ || exit 1
    else
        cp -t "$work_dir"/IPQ8074.ILQ.7.0/common/build/ipq/ $@ || exit 1
    fi
    echo >&2 "done"
fi
...
(
    cd "$work_dir"/IPQ8074.ILQ.7.0/common/build/
    if [ "$bits" = "64" ]; then
        sed '/debug/d;/packages/d;/"ipq807x"/d;/t32/d;/ret_prep_32image/d;/Required/d' -i update_common_info.py
        sed -i 's/.\/ipq/.\/ipq_x64/g' update_common_info.py
        sed -i 's/.\/ipq_x64_x64/.\/ipq_x64/g' update_common_info.py
    else
        sed '/debug/d;/packages/d;/"ipq807x_64"/d;/t32/d;/ret_prep_64image/d;/Required/d' -i update_common_info.py
        sed 's/"ipq807x"/"ipq807x_uboot"/g' -i update_common_info.py
    fi
...
if [ "$output_dir" ]; then
    ...
    cp -t "$output_dir" "$work_dir"/IPQ8074.ILQ.7.0/common/build/bin/*.img
    echo >&2 "done"
fi
