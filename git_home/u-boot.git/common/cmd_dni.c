...
#ifdef DNI_NAND
	ret = nand_read_skip_bad(&nand_info[0], BOARDDATA2_ADDR, &read_size, NULL, 0x20000, (u_char *)buffer);
	printf(" %zu bytes read: %s\n", read_size,
	       ret ? "ERROR" : "OK");
#endif
...
#ifdef DNI_NAND
	ret = nand_read_skip_bad(&nand_info[0], BOARDDATA2_ADDR, &read_size, NULL, 0x20000, (u_char *)sectorBuff);
	printf(" %zu bytes read: %s\n", read_size,
	       ret ? "ERROR" : "OK");
#else
#ifdef CONFIG_FUNC_MMC
...
int do_bootdni(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	...
	if (bootpart == 1)
	{
		...
		bootpart = boot_partition();
		if (bootpart == 1){
			snprintf(runcmd, sizeof(runcmd), "nand read 0x44000000 0x1980000 0x03200000");
			run_command(runcmd, 0);
		}
		if (bootpart == 2){
			printf("Loading DNI firmware for checking...\n");
			...
			bootpart = boot_partition();
			if (bootpart == 2){
				snprintf(runcmd, sizeof(runcmd), "nand read 0x44000000 0x4b80000 0x03200000");
				run_command(runcmd, 0);
			}
		}
	}
