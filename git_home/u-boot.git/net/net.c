...
extern uchar NetOurTftpIP[];
extern int NetRunTftpServer;
extern struct in_addr TftpClientIP;
#ifdef DNI_NAND
#include <nand.h>
#else
		...
		case TFTPPUT:
			...
			if(NetRunTftpServer)
			{
				...
				if (NmrpState != 0) {
					...
					net_copy_ip(&net_ip, NetOurTftpIP);
...
void net_process_received_packet(uchar *in_packet, int len)
{
	...
	switch (eth_proto) {
		...
		case PROT_NMRP:
			...
			struct in_addr zeroip = {0};
			(*udp_packet_handler)((uchar *)ip, 0, zeroip, 0, PROT_NMRP);
			break;
#endif
	case PROT_ARP:
