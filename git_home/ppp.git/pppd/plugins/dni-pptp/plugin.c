...
static int pptp_open_session(void)
{
	...
	dst_addr.sa_addr.pptp.sin_addr=conn->svr_addr;
	{
		int sock;
		...
		{
			close(sock);
			fprintf(stderr, "Unable to connect PPTP socket!\n");
			return -1;
		}
	...
	/* Need implement following code before connect, or it will always connect 
	 * failed later.
	 */
	...
	conn->gre_socket=socket(AF_PPPOX,SOCK_STREAM,PX_PROTO_PPTP);
	if(conn->gre_socket < 0)
		pptp_fatal("Failed to create PPTP socket: %m");

	if (bind(conn->gre_socket,(struct sockaddr*)&src_addr,sizeof(src_addr)))
	{
		close(conn->gre_socket);
		fprintf(stderr, "Unable to bind PPTP socket!\n");
		return -1;
	}
	len=sizeof(src_addr);
	getsockname(conn->gre_socket,(struct sockaddr*)&src_addr,&len);
	...
	dst_addr.sa_addr.pptp.call_id=conn->peer_call_id;

	if (connect(conn->gre_socket,(struct sockaddr*)&dst_addr,sizeof(dst_addr)))
	{
