...
struct dhcpOfferedAddr *add_lease(u_int8_t *chaddr, u_int32_t yiaddr, time_t lease)
{
...
{
	return (lease->expires < (u_int32_t) time(0));
}	
...
struct dhcpOfferedAddr *oldest_expired_lease(void)
{
	...
	u_int32_t oldest_lease = (u_int32_t)time(0);
	unsigned int i;
