...
/* This function is to create DHCP option 125 with 3 sub-options for Orange France Livebox4/
 * WHD94 decoder and Orange Spain ISP STB. VIinfo is where the option string stored.
 */
int createVIoption(int type, char *VIinfo)
{
	...
	*(unsigned int*)(optionData+VENDOR_OPTION_ENTERPRISE_OFFSET) = (unsigned int)VENDOR_ADSL_FORUM_ENTERPRISE_NUMBER;
	...
	/* update device OUI/Serial Number/Product Class for Orange France & Spain ISPs */
	if (strcmp(cat_file(ENABLE_ORANGE_FILE), "1") == 0)
	{
		if (strcmp(cat_file(LB4_DEV_OUI_FILE), "0") == 0)
		{
			strncpy(device_oui, (const char*)_LB4_deviceOui, OUI_LENGTH - 1);
		}
		else
		{
			strncpy(device_oui, cat_file(LB4_DEV_OUI_FILE), OUI_LENGTH - 1);
		}
		if (strcmp(cat_file(LB4_DEV_SN_FILE), "0") == 0)
		{
			strncpy(device_sn, (const char*)_LB4_deviceSerialNum, SN_LENGTH - 1);
		}
		else
		{
			strncpy(device_sn, cat_file(LB4_DEV_SN_FILE), SN_LENGTH - 1);
		}
		if (strcmp(cat_file(LB4_DEV_PC_FILE), "0") == 0)
		{	
			strncpy(device_pc, (const char*)_LB4_deviceProductClass, PC_LENGTH - 1);
		}
		else
		{
			strncpy(device_pc, cat_file(LB4_DEV_PC_FILE), PC_LENGTH - 1);
		}
	}
	else if (strcmp(cat_file(MOVISTAR_IPTV_FILE), "1") == 0)
	{
		strncpy(device_oui, (const char*)_ESMS_deviceOui, OUI_LENGTH - 1);
		strncpy(device_sn, (const char*)_ESMS_deviceSerialNum, SN_LENGTH - 1);
		strncpy(device_pc, (const char*)_ESMS_deviceProductClass, PC_LENGTH - 1);
	}
...
/* create specific DHCP option info for Spain Movistar IPTV services. */
int createSMOptInfo(int type, int code, char *OptInfo)
{
	char optionData[OPT_DATA_LEN] = {0}, *dataPtr = NULL;
	int len = 0;
	char buffer_pri[255] = {0}, buffer_sec[255] = {0};
	struct in_addr *addr_pri = (void *)buffer_pri;
	struct in_addr *addr_sec = (void *)buffer_sec;

	if (type != OPT_FOR_GATEWAY)
		return -1;

	optionData[OPT_CODE] = (char)code;
	dataPtr = optionData + 2;
	switch (code)
	{
		case DHCP_DNS_SERVER:
			/* Acoording to RFC, DNS option length must be a multiple of 4. 
			* Add specific server "172.26.23.3" and router IP as DNS servers.
			*/
			inet_aton((const char *)_SP_MOVISTAR_DNS, addr_pri);
			memcpy(optionData + 2, (void *)buffer_pri, 4);
			inet_aton(cat_file(LAN_IPADDR_FILE), addr_sec);
			memcpy(optionData + 6, (void *)buffer_sec, 4);
			optionData[OPT_LEN] = 8;
			break;
		case DHCP_OPT_PVT240:
			len = strlen(_SP_MOVISTAR_PVT240);
			strncpy(dataPtr, (const char*)_SP_MOVISTAR_PVT240, len);
			optionData[OPT_LEN] = len;
			break;
		default:
			break;
	}
	memcpy((void*)OptInfo, (void*)optionData, optionData[OPT_LEN] + 2);
