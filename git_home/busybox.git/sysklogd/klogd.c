...
int klogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (!bb_got_signal) {
		...
		while (1) {
			...
			/* Dni code in 2/26/2020 for fix timestamp on differen platform */
			/* Delete timestamp from kernel log */
			char *p_left_bracket = NULL, *p_right_bracket = NULL;
			int p_len = 0;
			p_left_bracket = strchr(start, '[');
			p_right_bracket = strchr(start, ']');
			p_len = p_right_bracket - p_left_bracket + 1;
			if(p_left_bracket != NULL &&
			   p_right_bracket != NULL &&
			   28 > p_len && p_len > 2)
			{
				/* maxval of 64bit 's len is 20 chars, str: %20lu.%6lu,
				 * then need 20 + 6 + 1 + \0= 27 */
				char str_temp[28] = {0};
				strncpy(str_temp, p_left_bracket, p_len);
				/* it's a timestamp, then skip it. */
				if(strspn(str_temp, "[ 0123456789.]") == p_len){
					printf("netgear kernel log:%d\r\n", p_len);
					/* skip [ and space chr */
					start = p_right_bracket + 2;
				}
			}
