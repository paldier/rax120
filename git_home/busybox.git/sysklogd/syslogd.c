...
unsigned int logorgsize = 128 * 1024;
#define LOG_ORG_DEL_NUM 200
...
#ifdef BUG_79944_AX6000
static int ntpclient_updated(void)
{
	//adjust ntp_updated is exist or not.
...
struct log_category reg_logs[]={
	...
	{"OpenVPN",                                            11},     /*  VPN */
	{"LAG",                                                13},     /*  Link aggregation */
	{"WAN aggregation",                                    13},     /*  WAN aggregation */
	{NULL,                                                  0},		 
};
/*
...
int generate_log_message(int flag,int info)
{
	...
	/*fix after clear log ,ntp updated,log will come back .bug:RAX227*/
    	char cmd[128];
	...
	while(fgets(buffer,LOG_LEN,fp)){
		...
		if(flag){
			...
			if(strncmp("[3G", log_message, 3) == 0)
				...
				sprintf(log_message_tmp, "%s %s\n", log_message, loc_time);
			}
		...
		fprintf(flog,"%s",log_message_tmp);
...
#else      
		//		ntp_flag = ntpclient_updated();
		//		ntp_flag = 1;
#endif

#ifdef BUG_79944_AX6000
				/*
				 * measure bug 79944, judge the ntpclient has calibrated or not 
				 * then get the flag value. 
				 **/
				ntp_flag = ntpclient_updated();
#endif
				/*regenarate log if ntp's time value has changed*/
				 ...
				 * foramer process,the log will also be printed once again,so we do some changes here.
				 * */
				else if(ntp_flag == 2){
					return;
				}
				else if(ntp_flag == 0){ /*delete unwanted comma since no time precedes*/   ///  by -walleva
			     	check_log_len(msg,0);
					if(mobile_log == 1){
...
int syslogd_main(int argc UNUSED_PARAM, char **argv)
{
	...
	//	update_log(logcy);  //#bug_79944_ax6000,this function is unnecessary,the demand will do by function do_syslogd();
	}
#endif
