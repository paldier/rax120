...
function close_all_sub(click_name)/* fold all menus, except the menu which user click */
{
	if(!IsGameRouter())
		...
		var sub_list = "setup,usb,security,green,admin,advanced,moniter,plex_media";
	...
	for ( i=0; i< sub_array.length; i++)
	{
		...
		if( sub_name != click_name )
		{
			...
			if(getElementFromTop("document").getElementById(sub_name))
				getElementFromTop("document").getElementById(sub_name).style.display="none";
		}
	}
}
...
function open_or_close_sub(name)
{
	...
	if(getElementFromTop("document").getElementById(sub_name))
	{
		var open_flag= getElementFromTop("document").getElementById(sub_name).style.display;
	...
	else
	{
		if(sub_exist){
			...
			if(button_div.className.indexOf("advanced_white_close_button") !=-1)
				settingClass(button_div, "advanced_white_open_button");
			else
				settingClass(button_div, "advanced_white_close_button");	
		}
...
function subItemsClass(argv)
{
	...
	for(num=0; num<arguments.length; num++)
	{
		...
		{
			if( getElementFromTop("old_div")== items[i] ) continue;
...
var ap_items = [ "internet", "wan", "block_site", "block_services", "forwarding_triggering","qos","vpn", "remote", "upnp", "traffic", "wl_bridge", "lan", "ipv6", "parental", "devname", "vpn_client"];
...
var game_router_array_name = ["game_router_wds_items", "game_router_ap_items", "game_router_bridge_items", "game_router_extender_items", "game_router_mapt_items", "game_router_devmode_items"];
var game_router_enable_flags = ["enabled_wds", "enable_ap_flag", "enable_bridge_flag", "enable_extender_flag", "enable_mapt", "device_mode"];
var game_router_wds_items = [ "vlan","internet", "adv_setup_wizard", "adv_wps", "wan", "block_site", "block_services", "forwarding", "triggering","qos","vpn", "dns", "static", "remote", "upnp", "traffic", "wl_bridge", "guest", "email", "schedule", "log"]; 
var game_router_ap_items = ["vpn_client", "vlan", "internet", "adv_setup_wizard","wan", "block_site", "block_services", "forwarding", "triggering","qos","vpn", "remote", "upnp", "traffic", "wl_bridge", "lan", "ipv6", "parental", "access_control", "conn_status"];
var game_router_extender_items = [ "vlan","internet", "adv_setup_wizard", "wan", "block_site", "block_services", "forwarding", "triggering","vpn",  "remote", "upnp", "traffic", "wl_bridge", "lan", "ipv6", "led", "fbwifi","conn_status"];
var game_router_bridge_items = ["vlan","internet", "adv_setup_wizard", "adv_wps", "wan", "block_site", "block_services", "forwarding", "triggering","qos","vpn", "dns", "static", "remote", "upnp", "traffic", "wireless", "guest", "lan", "parental","access_control",  "email", "schedule", "wds", "ipv6", "wlap"];
var game_router_mapt_items= [ "internet" ,"adv_setup_wizard" ];
var game_router_devmode_items = [ "internet", "adv_setup_wizard", "adv_wps", "wireless", "guest", "wan", "lan", "qos", "readyshare_stor", "usb_bas", "usb_adv", "media", "print", "vault", "block_site", "block_services", "schedule", "email", "wladv", "rae", "forwarding", "triggering", "dns", "vpn", "static", "remote", "upnp", "traffic", "wl_bridge", "ipv6", "parental", "access_control", "usb_set", "vlan", "led", "devname", "plex_media" ];
...
function enabledItemsClass()
{
	...
	var larray;
	if(IsGameRouter())
		larray=game_router_array_name.length;
	else
		larray=array_name.length
	...
	for(i=0; i<larray; i++)
	{
		...
		for(j=0; j<item_group.length; j++)
		{
			...
			if(enable_flag == "1" || (master != "admin" && item_group[j] == "upgrade"))
			{
				if(!IsGameRouter()){
					if(height == 38 ||height == 52)
						height=height-10;
					if( height > 28 )
	...
	{
		var cur_div = getElementFromTop("document").getElementById("mobile_internet");
		var cur_div_1= getElementFromTop("document").getElementById("internet");
		var height = cur_div.getElementsByTagName("a")[0].clientHeight;
...
function menu_class_default2()
{
	...
	var content_length;
        var last_div;
    var extensible_items = ["setup_bt", "usb_bt", "security_bt", "green_bt", "admin_bt", "advanced_bt", "moniter_bt", "plex_media_bt"];
	var i;
	
	
	//first page open internet. when old_div is blank, change class to default
	last_div = top.old_div;
	if( last_div ===  undefined || last_div == "")
	{
...
function menu_color_change( change_id )
{
	...
	{
		var parent_id = getElementFromTop("document").getElementById(change_id).parentNode.parentNode.id;
                var btn_id = parent_id.replace('sub', 'bt');
                var btn_div = getElementFromTop("document").getElementById(btn_id);
	...
	if(id == "adv_setup_wizard")
		...
		for(var i=0; i<larray; i++)
		{
			...
			else
			{
				...
				var item_group = eval(game_router_array_name[i]);		
			}
		...
		if( id == "adv_home" )
			...
			if(enable_bridge_flag == "1")
                ...
                else if( id == "adv_setup_wizard" && master == "admin" && getElementFromTop("enable_ap_flag") != "1" && getElementFromTop("enabled_wds") != "1" && getElementFromTop("enable_bridge_flag") != "1" && getElementFromTop("enable_extender_flag") != "1" && getElementFromTop("device_mode") != "1")
		{
			menu_color_change('adv_setup_wizard');
			if(getElementFromTop("dsl_enable_flag") == "1")
				goto_formframe('DSL_WIZ_sel.htm');
			else
			   if (((getElementFromTop("netgear_region").toUpperCase() == "WW" || getElementFromTop("netgear_region") == "") && getElementFromTop("gui_region") == "Russian" ) || getElementFromTop("netgear_region").toUpperCase() == "RU")
				goto_formframe('RU_WIZ_sel.htm');
			  else
				goto_formframe('WIZ_sel.htm');
		}
                
                else if( id == "adv_wps" && getElementFromTop("enabled_wps") == "1" && getElementFromTop("enabled_wds") != "1" && getElementFromTop("enable_bridge_flag") != "1" && getElementFromTop("device_mode") != "1" )
		{
			...
			menu_color_change('adv_wps');
                ...
                {
			if(!(getElementFromTop("have_lte_flag")==1 && getElementFromTop("broadband_wan_type") == "MyDetc"))
			{
				if(getElementFromTop("dsl_enable_flag") == 0)
		...
		{
			if(!IsGameRouter()&&top.trend_micro_qos == "1") {
                                please_wait(false);
                                top.formframe.location.href = "new_waiting_page.htm";
                        }
			else if(getElementFromTop("have_advanced_qos") == "1")
				goto_formframe("QOS_wait.htm");
			else if(!IsGameRouter()&&top.trend_micro_flag == "1") {
				please_wait(false);
				top.formframe.location.href = "new_waiting_page.htm";
			}
			else
		...
		else if(id == "plex_media")
		{
			goto_formframe("plex_media.htm");
			open_or_close_sub("plex_media");
		}
		else if( id == "cloud" )
		{
		...
		else if( id == "ipmac")
		{
			...
			menu_color_change('ipmac');
                ...
                {
			if(getElementFromTop("have_lte_flag") ==1)
			...
			menu_color_change('log');
                ...
                {
			if(getElementFromTop("have_dynamic_qos") == 1 || getElementFromTop("use_orbi_style_flag") == 1)
			{
			...
			menu_color_change('passwd');
                ...
                else if( id == "upgrade" && master == "admin")
                {
		...
		else if( id == "lte_broadband" && getElementFromTop("have_lte_flag")==1)
		{
			goto_formframe("BAS_lte_broadband.htm");
			menu_color_change(id);
		}
		else if( id == "mobile_internet" && getElementFromTop("broadband_wan_type") != "AutoDetc")
		{
			if(!IsGameRouter())
				top.click_lte_sim = 0;
			else
		...
		{
			//getElementFromTop("click_lte_sim")=1;
			if(!IsGameRouter())
				top.click_lte_sim = 1;
			else
				click_lte_sim = 1;
		...
		{
			if(getElementFromTop("vpn_smartphone_flag"))
		...
		else if( id == "vpn_client" && getElementFromTop("vpn_client_flag") == "1")
		{
			goto_formframe("vpn_client.htm");
			menu_color_change('vpn_client');
		}else if(IsGameRouter()&& id == "statistics")
		{
			goto_formframe("RST_statistic.htm",false);
			menu_color_change('statistics');
		}else if(IsGameRouter()&& id == "reboot")
		{
			goto_formframe("FW_reboot.htm",false);
			menu_color_change(id);
		}else if(IsGameRouter()&& id == "conn_status")
		{
			/*if(select_basic == "0" && select_type == "3")
				goto_formframe("RST_multipppoe_conn_status.htm",false);
			else
				goto_formframe("RST_conn_status.htm",false);*/
			goto_formframe("Change_RST_status.htm",false);
