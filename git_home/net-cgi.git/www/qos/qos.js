...
function qosmac_data_select(num)
{
	...
	cf.priority.value=convert_value(cf, "priority", each_info[1]);
...
function attached_qosmac_data_select(num,j)
{
	...
	cf.priority.value=convert_value(cf, "priority", "2");
	cf.select_editnum_mac.value=num;
}
...
//add new info	
	...
	cf.hid_category.value=convert_value_to_original(cf, "category");
	cf.hid_priority.value=convert_value_to_original(cf, "priority");
	cf.submit();
	return true;
}
...
{
	if((cf.bwEnable.checked == true) && quick_qos ==1 && fast_lane == 2)
	{
	...
	qos_bandwith=cf.tcbw_value.value;
	if ( cf.bwEnable.checked == true )
	{
		var qos_bandwith_alert = ("$qos_bandwith1000M").replace("1000",max_bandwidth);
		if(max_bandwidth*1024 > 999999)
			var max_bandwidth_kbps = 999999;
		else
			var max_bandwidth_kbps = max_bandwidth*1024;
		var qos_bandwith_alert_kbps = ("$qos_bandwith1000").replace("1000",max_bandwidth_kbps);
		if(cf.tcbw_unit.selectedIndex == 0)
		{
			if(!(qos_bandwith > 0.99 && qos_bandwith < (max_bandwidth_kbps+0.01)))
			{
				alert(qos_bandwith_alert_kbps);
				return false;
			}
			else
				cf.qos_hidden_uprate.value = cf.tcbw_value.value.substr(0,6);
		}
		else
		{
			if(!(qos_bandwith > 0.99 && qos_bandwith <(max_bandwidth + 0.01)))
			{
				alert(qos_bandwith_alert);
				return false;
			}
			else
				cf.qos_hidden_uprate.value = cf.tcbw_value.value*1024;
		}
	}
	else
	{
