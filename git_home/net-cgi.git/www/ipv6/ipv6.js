...
function login_type_show(cf)
{
	...
	var ipv6_auto_detect = getTop(window).ipv6_auto_detect;
	var ipv6_dhcp_flag = getTop(window).ipv6_dhcp_flag;
	var ipv6_pppoe_flag = getTop(window).ipv6_pppoe_flag;
	...
	for(i=0; i<ipv6_items.length; i+=2)
	{
		if((ipv6_6rd_flag != 1 && ipv6_items[i] == "6rd") || (ipv6_auto_detect != 1 && ipv6_items[i] == "autoDetect") || (ipv6_dhcp_flag != 1 && ipv6_items[i] == "dhcp") || (ipv6_pppoe_flag != 1 && ipv6_items[i] == "pppoe"))
			continue;
		else if(getTop(window).support_orange_flag ==1 && ipv6_items[i] == "orange") {
