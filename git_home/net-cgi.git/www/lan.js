...
function checklan(form)
{
	...
	if(lease_time=="" || !_isNumeric(lease_time) || parseInt(lease_time) < 1 || parseInt(lease_time) > 24) {
...
*/
	...
	if( change_count > 0)
		alert("$change_ip_manually1"+" After this new IP change, the corresponding configurations (DMZ, port forwarding/triggering, block service and block site trusted IP, Address Reservation Table) will be flushed too.");
	cf.submit();//add
	return true;
}
