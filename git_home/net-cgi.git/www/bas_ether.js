...
function check_wizard_dhcp(check,page)
{
	...
	//modify according to bug98964
	var pattern=/(?=^.{1,255}$)(^[a-zA-Z]([-a-zA-Z0-9]{0,61}[a-zA-Z0-9])?(\.[a-zA-Z]([-a-zA-Z0-9]{0,61}[a-zA-Z0-9])?)*$)/;
	if(cf.domain_name.value != "" &&(cf.domain_name.value.match(pattern)==null || cf.domain_name.value.match(pattern)[0]!=cf.domain_name.value)){
...
function check_ether(cf,check)
{
	...
	{
		if(cf.wan_preference[4].checked == true)
		{
			...
			else if(aggre_option != "0" && wan_agg_port != 4)
			{
	...
	{
	if(cf.enable_ds_lite.checked == true)
	{
		...
		cf.hid_enable_ds.value=0;
		if(cf.enable_nd_proxy.checked == true)
