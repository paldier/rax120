...
function check_wizard_pptp(check,page)
{
	...
	cf.hid_l2tp_dod.value=convert_value_to_original(cf, "l2tp_dod");
	if(cf.hid_l2tp_dod.value == "1")
	{
	...
	var is_domain_name=0;
    	if( serv_array.length==4 )
    	{
        	...
        	if( flag == 4 && (is_IP_addr(serv_array)==true) && (checkipaddr(cf.l2tp_serv_ip.value) == true))//if server address is number and length is 4,check it
        	{
			/* bug 31100 */
				if ( cf.l2tp_myip.value !="" )
				{
...
function check_l2tp(cf,check)
{
	...
	cf.hid_l2tp_dod.value=convert_value_to_original(cf, "l2tp_dod");
	if( cf.hid_l2tp_dod.value == "1" || cf.hid_l2tp_dod.value == "2" )
	{
