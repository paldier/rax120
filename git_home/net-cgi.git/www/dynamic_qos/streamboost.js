...
function check_qos_apply3()
{
	...
	var invalid = 0;
	if(isNaN(streamboost_downlink) || isNaN(streamboost_uplink)) {
		invalid = 1;
	}
	else {
		cf.downlink_value.value = streamboost_downlink;
		cf.uplink_value.value = streamboost_uplink;
	}
	...
	if(cf.qosSetting[1].checked == true) {
		...
		else if(streamboost_uplink<0.10 || streamboost_uplink>1000.00 || streamboost_downlink<0.10 || streamboost_downlink>1000.00 || invalid == 1)
		{
			sAlert("$range_error");
			return false;
		}
