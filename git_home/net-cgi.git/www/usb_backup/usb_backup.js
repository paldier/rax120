...
function click_apply() {
	...
	cf.hid_backup_day.value = day.replace(/7/g, "0");
...
function print_drive_info(flag) {
	...
	if(eval(flag + "_sel_hdd_id") == "" && eval(flag + "_hdd_id") != "")
		var mark_hdd_id = eval(flag + "_hdd_id");
	else
		var mark_hdd_id = eval(flag + "_sel_hdd_id");
	...
	for (i = 0, j = 0; i < usb_drive_num; i++) {
		...
		if (mark_hdd_id == each_info[0])
		{
			...
			eval(flag + "_sel_hdd_id = mark_hdd_id");
		}
		else
	...
	if((usb_drive_num == 1 && jump == 1) || usb_drive_num == 0)
		tableHTML = "";
...
function open_error()
{
	var message = '<table cellpadding=0 cellspacing=2  border=0><tr><td colspan=3>'
	if(error_type == "")
	{
		...
		message += 'Your Primary Drive has been removed, please plug it back or select another one.';
	else if(error_type == "2")
		message += 'Your Backup Drive has been removed, please plug it back or select another one.';
	else if(error_type == "3")
		message += 'Your drive is invalid, please make sure you have permission to write.';
	else if(error_type == "4")
		message += 'Your Backup Drive space is not enough, please delete some files or select a new one.';
	else if(error_type == "5")
		message += 'The files you operated are invalid or have no permission to write.';
	else if(error_type == "6")
		message += 'Unknow error!';
	message += '</td></tr></table>';
	sAlert(message, function(){remove_msg();}, "", 475, 1, "Close");
}
