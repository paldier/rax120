function select_lable(num)
{
	if(top.enable_action == 0)
        ...
        if(num==1) {
                if(this.location.href.indexOf("QOS_dynamic.htm") != -1 && (first_flag == 1 || (first_flag == 0 && parent.ookla_speedtest_flag == 1)))
			return;
...
function check_wmm_apply(cf)
{
	...
	if(getTop(window).tri_router_flag == 1)
	{
		if(cf.wmm_enable_5g_2nd.checked == true)
...
function enable_links()
{
	...
	if(top.location.href.indexOf("adv_index.htm") > -1){
		...
		if(parent.window.document.getElementById('setup_sub').style.display == "none")
...
function grayout_menu_links()
{
	...
	if(top.location.href.indexOf("adv_index.htm") > -1){
		getTop(window).enabledItemsClass();
		parent.window.document.getElementById("adv_home").className = "advanced_grey_button_double";
		parent.window.document.getElementById("adv_setup_wizard").className = "advanced_grey_button_double";
		parent.window.document.getElementById("adv_wps").className ="advanced_grey_button_double";
	}
	else {
...
function edit_select_device_fing(mac,ip,name,priority,devtype,devtype_name,model,contype ){
	var cf = document.forms[0];

	if( mac ) parent.qos_edit_mac=mac;
	if( ip ) parent.qos_edit_ip=ip;
	if( name ) parent.qos_edit_name=name;
	else parent.qos_edit_name="";
	if( priority ) parent.qos_edit_priority=priority;
	if( devtype )parent.qos_edit_devtype=devtype;
	if(getTop(window).support_new_ntgrtype == "1")
	{
		if( devtype_name )parent.qos_edit_devtype_name=devtype_name;
		if( model )parent.qos_edit_model=model;
		else parent.qos_edit_model="";
	}
	if( contype )parent.qos_edit_contype=contype;
