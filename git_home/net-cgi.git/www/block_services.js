...
{
	if(is_jp_version == 1 && wan_type == "mulpppoe1" && enable_multipppoe_serv != "0")
	...
	if(cf.iptype[0].checked == true)
	{
		/*for(i=1;i<=array_num;i++)
	        ...
	        if(checkipaddr(cf.only_ip.value)==false || is_sub_or_broad(cf.only_ip.value, lan_ip, lan_subnet) == false)
		{
			alert("$invalid_ip");
			return false;
		}
	...
	{
		if(is_jp_version == 1 && wan_type == "mulpppoe1" && enable_multipppoe_serv != "0")
				var str = eval ( 'session2_block_servicesArray' + i ).replace(/&lt;/g,"<").replace(/&gt;/g,">").replace(/&#40;/g,"(").replace(/&#41;/g,")").replace(/&#34;/g,'\"').replace(/&#39;/g,"'").replace(/&#35;/g,"#").replace(/&#38;/g,"&");
		else
...
{
	if(is_jp_version == 1 && wan_type == "mulpppoe1" && cf.session[1].checked == true)
		array_num = session2_array_num;
	if( array_num == 0 )
	{
