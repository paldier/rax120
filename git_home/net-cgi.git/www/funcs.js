...
function sAlert2(str, callback_ok, callback_cancel, dwidth, anc, button_lang){
	...
	function removeObj(){//close warning
		...
		document.getElementById("msgDiv").parentNode.removeChild(document.getElementById("msgDiv"));//delete warning
	}
...
function please_wait(flag)
{
	...
	if ( flag ) {
		...
		parent.timeoutfunc=parent.setTimeout("please_wait(false);", 30000)
	} else {
		if(parent.timeoutfunc)
			parent.clearTimeout(parent.timeoutfunc);
...
function mtu_change(wanpro)
{
	...
	else if((old_wan_type=="pppoe" || old_wan_type=="orange_pppoe")&& wan_mtu_now!=1492)
	{
...
function default_mtu(wanpro)
{
	...
	else if(wanpro=="pppoe" || wanpro=="orange_pppoe")
...
{
	if(!IsGameRouter())
		return checkpsk1(passphrase,passphrase_len);
	else
		return checkpsk2(passphrase,passphrase_len);
}
...
{
	if(typeof(notallowpassps) == "undefined")
		notallowpassps = "";
	if(typeof(passphrase_short8) == "undefined")
		passphrase_short8 = "";
	if(typeof(passphrase_long63) == "undefined")
		passphrase_long63 = "";
	var len = passphrase.value.length;
	if ( len == 64 )
	{
		for(i=0;i<len;i++)
		{
			if(isValidHex(passphrase.value.charAt(i))==false)
			{
				alert(getText(notallowpassps, "$notallowpassps"));
				return false;
			}
		}
	}
	else
	{
		if(len < 8 )
		{
			alert(getText(passphrase_short8, "$passphrase_short8"));
			return false;
		} 
		if(len > 63){
			alert(getText(passphrase_long63, "$passphrase_long63"));
			return false;
		}
		for(i=0;i<passphrase.value.length;i++)
		{
...
function checkpsk128(passphrase, passphrase_len)
{
	...
	if(typeof(passphrase_short8) == "undefined")
		passphrase_short8 = "";
	if(typeof(passphrase_long127) == "undefined")
		passphrase_long127 = "";
	var len = passphrase.value.length;
	if ( len == 128 )
	{
		for(i=0;i<len;i++)
		{
			if(isValidHex(passphrase.value.charAt(i))==false)
			{
				alert(getText(notallowpassps, "$notallowpassps".replace("63","127").replace("64","128")));
				return false;
			}
		}
	}
	else
	{
		...
		if(len > 127){
			alert(getText(passphrase_long127, "$passphrase_long63".replace("63","127")));
			return false;
		}
		for(i=0;i<passphrase.value.length;i++)
		{
...
*/
	...
	else if(getTop(window).support_orange_flag ==1 && cf.login_type.value == "Orange PPPoE" )
		goto_formframe("BAS_orange_pppoe.htm");
	else if(getTop(window).support_orange_flag ==1 && cf.login_type.value == "Orange DHCP" )
		goto_formframe("BAS_orange_dhcp.htm");
	else if(getTop(window).support_singapore_isp_flag == 1 && cf.login_type.value == "SingTel Singapore DHCP")
		goto_formframe("BAS_singtel_singa_dhcp.htm");
	else if(getTop(window).support_malaysia_isp_flag == 1 && cf.login_type.value == "Unifi Malaysia DHCP")
		goto_formframe("BAS_unifi_malaysia_dhcp.htm");
	else if(getTop(window).support_malaysia_isp_flag == 1 && cf.login_type.value == "Maxis Malaysia DHCP")
		goto_formframe("BAS_maxis_malaysia_dhcp.htm");
	else if(getTop(window).support_malaysia_pppoe_isp_flag == 1 && cf.login_type.value == "Unifi Malaysia PPPoE")
		goto_formframe("BAS_unifi_malaysia_pppoe.htm");
	else if(getTop(window).support_malaysia_pppoe_isp_flag == 1 && cf.login_type.value == "Maxis Malaysia PPPoE")
		goto_formframe("BAS_maxis_malaysia_pppoe.htm");
	else if(getTop(window).support_spain_isp_flag == 1 && cf.login_type.value == "Orange Spain DHCP")
		goto_formframe("BAS_orange_spain_dhcp.htm");
	else if(getTop(window).support_spain_isp_flag == 1 && cf.login_type.value == "Movistar Spain PPPoE")
		goto_formframe("BAS_movi_spain_pppoe.htm");
	else if(getTop(window).support_spain_isp_flag == 1 && cf.login_type.value == "Vodafone Spain PPPoE")
...
function wan_aggregation_status() {
	var statusStr = "";
	var el = document.getElementById("wan-port-status1");
	for(i=2; i<=array_num; i++)
	{
		...
		var zero="";
		for(u=0; u<(parent.lan_ports_num+1-tmp_lan.length); u++)
			zero = zero + "0";
		var lan_ports = (zero + tmp_lan).split("");
		if(lan_ports[parent.lan_ports_num] == 1 || bri_lan_tag1 == 1)
		{
	...
	else if(wan_aggre == "no" || wan_aggre == "Invalid bond id" || wan_internet_cable === "Link down")
	{
		statusStr = "$wan_aggregation_error1";
		el.innerHTML = statusStr;
		el.style.color = "red";
	}
	else if(wan_aggre == "config")
	{
		statusStr = "$wan_aggregation_error2";
		el.innerHTML = statusStr;
		el.style.color = "red";
...
function have_wan_preference(flag) {
	...
	else if(getTop(window).use_orbi_style_flag == "1"){
...
var pptp_dod_list = l2tp_dod_list = pppoe_dod_list = mulpppoe_demand_list = {
	"0": ["0", "Always On"],
	"1": ["1", "Dial on Demand"],
...
var allow_or_block_list={
	"0": ["Allowed", "Allow"],
	"1": ["Blocked", "Block"]
};

var cfAlert_Select_list={
	"0": ["0", "full"],
	"1": ["1", "Hourly"],
	"2": ["2", "Daily"],
	"3": ["3", "Weekly"],
...
function rewrite_list_value(cf, obj_name)
{
	...
	var obj=eval("cf."+ obj_name);
	if(window[list_name] == undefined)
		return;
	console.log('rewrite_list_value():', obj);
	for(var i in window[list_name])
	{
		for(var j=0; j<obj.options.length; j++)
		{
			if(window[list_name][i][0]==obj.options[j].value)
			{
				obj.options[j].value=window[list_name][i][1];
				console.log("[rewrite_list_value]",j, " current_value:",window[list_name][i][0],", rewrite_value:",obj.options[j].value);
			}
		}
	}
}
...
function rewrite_list_value_to_original(cf, obj_name)
{
	...
	if(window[list_name] == undefined)
		return;
	console.log('rewrite_list_value_to_original():', obj);
	for(var i in window[list_name])
	{
		for(var j=0; j<obj.options.length; j++)
		{
			if(window[list_name][i][1]==obj.options[j].value)
			{
				obj.options[j].value=window[list_name][i][0];
				console.log("[rewrite_list_value_to_original]",j, " current_value:",window[list_name][i][1],", rewrite_value:",obj.options[j].value);
			}
		}
	}
}
...
function convert_value(cf, obj_name, current_value)
{
	...
	for(var i in window[list_name])
	{
		...
		{
			console.log("[convert_value]",i, " current_value:", current_value,", convert_value:",window[list_name][i][1]);
			return window[list_name][i][1];
		}
	}
	return current_value;
}
...
function convert_value_to_original(cf, obj_name)
{
	...
	for(var i in window[list_name])
	{
		...
		{
			console.log("[convert_value_to_original]",i, " current_value:", current_value,", convert_value:",window[list_name][i][0]);
			return window[list_name][i][0];
		}
	}
	return current_value;
}
