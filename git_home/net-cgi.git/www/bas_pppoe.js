...
function check_wizard_pppoe(check,servername,page)
{
	...
	if(convert_value_to_original(form, "pppoe_dod") == "1")
	{
...
function check_pppoe(form,check)
{
	...
	if(getTop(window).support_ds_lite_flag === 1 && is_jp_version)
	{
		if(form.enable_ds_lite.checked != true){
			if(check_wizard_pppoe(check,form.pppoe_servername.value,"bas")==false)
				return false;
		}
	}else if(check_wizard_pppoe(check,form.pppoe_servername.value,"bas")==false)
		return false;
	form.hidden_pppoe_idle_time.value = form.pppoe_idletime.value;
	form.hid_pppoe_dod.value = convert_value_to_original(form, "pppoe_dod");//bug 22322:when connection mode gray out,need a hidden-value to transfer
	...
	if(!(getTop(window).support_ds_lite_flag === 1 && is_jp_version && form.enable_ds_lite.checked == true)){
	if( form.hid_pppoe_dod.value == "1" || form.hid_pppoe_dod.value == "2" )
	{
	...
	{
		if(form.wan_preference[4].checked == true)
		{
			if(qos_endis_on != "0" && aggre_option != "0")
			{
				if(confirm("Enabling WAN aggregation disables QoS and Ethernet Port Aggregation. Do you want to enable WAN aggregation?") == false)
					return false;
			}
			else if(qos_endis_on != "0")
			{
				if(confirm("Enabling WAN aggregation disables QoS. Do you want to enable WAN aggregation?") == false)
					return false;
			}
			else if(aggre_option != "0")
			{
				if(confirm("Enabling WAN aggregation disables Ethernet Port Aggregation. Do you want to enable WAN aggregation?") == false)
					return false;
			}
		}
	}
	...
	if(form.enable_ds_lite.checked == true)
	{
		if(ipv6_type == "disabled" || ipv6_type == "6to4"|| ipv6_type == "bridge")
			alert("IPv6 connection type will be set to \"Auto Config\" automatically");
		form.hid_enable_ds.value=1;
		if(form.hid_aftr_ipv6.value == "1"){
	if(form.aftr_ipv6_addr.value == "")
	{
		alert("Empty Manual Config AFTR IPv6 Address!");
		return false;
	}
	...
	var is_fqdn_addr = /^(www\.)?[-a-zA-Z0-9]+(\.[-a-zA-Z0-9]+)+$$/g.test(tmp);
	if(isIpv6IP)
	{
		if(ipv6_array.length<4 || ipv6_array.length>8)
		{
			alert("Invalid Manual Config AFTR IPv6 Address!");
			return false;
		}
	}
	else if(!is_fqdn_addr)
	{
		...
		form.hid_enable_ds.value=0;
		if(form.enable_nd_proxy.checked == true)
...
{
	if(!((getTop(window).support_singapore_isp_flag == 1 && form.login_type.value == "SingTel Singapore DHCP") ||
		(getTop(window).support_malaysia_isp_flag == 1 && form.login_type.value == "Unifi Malaysia DHCP") ||
		(getTop(window).support_malaysia_isp_flag == 1 && form.login_type.value == "Maxis Malaysia DHCP")))
	{
