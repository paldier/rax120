...
function check_wizard_pptp(check,page)
{
	...
	if(convert_value_to_original(cf, "pptp_dod") == "1")
	{
	...
	if( (serv_array.length==4) && (is_IP_addr(serv_array)==true) )
	{
    	...
    	if( serv_array.length==4 )
    	{
        	...
        	if((flag == 4) && (is_IP_addr(serv_array)==true) && (checkipaddr(cf.pptp_serv_ip.value) == true))
        	{
				if( cf.pptp_myip.value != "" )
				{
...
function check_pptp(cf,check)
{
	...
	cf.hid_pptp_dod.value = convert_value_to_original(cf, "pptp_dod");//bug 22322:when connection mode gray out,need a hidden-value to transfer
	...
	if( cf.hid_pptp_dod.value == "1" || cf.hid_pptp_dod.value == "2" )
	{
