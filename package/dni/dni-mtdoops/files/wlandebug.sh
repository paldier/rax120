...
usage(){
    ...
    echo "  -c [0|1] set/clear QualComm debug flag"
    echo "  -h [0|1] set/clear hostapd debug flag"
    echo "  -q [0|1] set/clear Quantenna debug flag"
}
...
while getopts "b:a:w:c:h:q:" opt; do
    case "$opt" in
        b)
...
basic_wlan_info_func(){
        ...
        if [ $1 -eq 1 ]; then
            ...
            for vap in $vif_ap_list; do
                ...
                print_section_hd "$(date -R)" >> /dev/console
                echo "[VAP: $vap]" >> /dev/console
                iwpriv $vap txrx_fw_stats 3 # cannot redirect to file
                iwpriv $vap txrx_fw_stats 6 # cannot redirect to file

                channel=$(iwlist $vap channel | grep Frequency | \
                          awk '{print $5}' | sed 's/)$//')
                echo "[VAP: $vap] setchanlist channel=$channel" >> "$2"
                wifitool $vap setchanlist $channel
                iwpriv $vap acsreport 1
                sleep 1
                wifitool $vap acsreport >> "$2"
                wifitool $vap setchanlist 0
...
do
        ...
        advanced_sys_info_func $advanced_sys_info /tmp/wireless_log${file_num}.txt
