...
#define VOLUME_FORMAT_FILE "/tmp/disk_format"
	...
	snprintf(cmd, sizeof(cmd), "/usr/sbin/blkid /dev/%s | grep 'TYPE=.*' | awk -F\\\" '{print $2}' > " VOLUME_FORMAT_FILE, dev);
	system(cmd);
...
#define DISK_LOOP_PARTITION "/tmp/disk_loop_pt"
       ...
       diskname[3] = '\0';
       snprintf(cmd, sizeof(cmd), "/usr/sbin/blkid /dev/%s | grep 'PTTYPE=.*' | awk '{print $2}' > " DISK_LOOP_PARTITION, diskname);
       system(cmd);
...
static int is_noshare_partition(char *dev)
{
	...
	snprintf(cmd, sizeof(cmd), "echo "" | grep %s", diskname, dev);
	fp = popen(cmd, "r");
	if (!fp) {
		perror("popen");
		return 0;
