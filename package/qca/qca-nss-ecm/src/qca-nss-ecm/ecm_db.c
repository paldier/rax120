...
int ecm_db_connection_deref(struct ecm_db_connection_instance *ci)
{
	...
	if (!ci->flags & ECM_DB_CONNECTION_FLAGS_INSERTED) {
		 ...
		 */
		if (ci->defunct_timer.group == ECM_DB_TIMER_GROUPS_CONNECTION_DEFUNCT_RETRY_TIMEOUT)
		{
			pr_emerg("Eric debug: ci %p defunct_timer.group is 23, need remove it anyway\n", ci);
