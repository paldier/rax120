...
{
	ssize_t hdr_sz = (inner->ip_version == IPVERSION) ? sizeof(inner->dest_ip[0]) : sizeof(inner->dest_ip);
...
{
	ssize_t hdr_sz = (outer->ip_version == IPVERSION) ? sizeof(outer->dest_ip[0]) : sizeof(outer->dest_ip);
